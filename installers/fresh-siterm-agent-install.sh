...
workdir=`pwd`
packages="git autoconf automake sudo libcurl-devel libffi-devel libxslt libxml2 openssl-devel curl gcc traceroute libmnl-devel libuuid-devel lm_sensors ipset make MySQL-python nc pkgconfig python3 python-psycopg2 PyYAML zlib-devel python3-devel wget vconfig tcpdump jq iproute cronie python3-pip wireshark iperf3"
