...
from DTNRMLibs.MainUtilities import (
    createDirs,
    getFullUrl,
    contentDB,
...
class Ruler:
    ...
    def setHostState(self, state, deltaid):
        ...
        restOut = getDataFromSiteFE(
            {},
            self.fullURL,
            "/sitefe/v1/deltas/%s/internalaction/%s/%s"
            % (deltaid, self.hostname, state),
        )
        if restOut[1] >= 400:
            self.logger.info(
                "Failed to set new state in database for %s delta \
                             and %s hostname. Error %s "
                % (deltaid, self.hostname, restOut)
            )
            raise FailedInterfaceCommand(
                "Failed to set new state in database for %s delta \
                                         and %s hostname. Error %s "
                % (deltaid, self.hostname, restOut)
            )
    ...
    def checkAllFiles(self):
        ...
        for fileName in glob.glob("%s/*.json" % self.workDir):
            ...
            if not deltaInfo:
                self.logger.debug(
                    "FE did not return anything for %s" % inputDict["uid"]
                )
                continue
            if deltaInfo[0]["state"] in [
                ...
                if os.path.isfile(fileName):
                    os.remove(fileName)
            elif deltaInfo[0]["state"] in ["activating", "activated"] and deltaInfo[0][
                "deltat"
            ] in ["addition", "modify"]:
    ...
    def checkHostStates(self):
        ...
        for state in states:
            ...
            if not deltaInfo:
                self.logger.debug(
                    "FE did not return anything for %s" % state["deltaid"]
                )
                continue
            if deltaInfo[0]["state"] in ["remove", "removed"] and deltaInfo[0][
                "deltat"
            ] in ["addition", "modify"]:
                ...
                deltaInfo[0]["state"] in ["remove", "removed"]
                ...
                deltaInfo[0]["state"] in ["activated"]
                and deltaInfo[0]["deltat"] == "reduction"
            ):
    ...
    def addition(self, deltaInfo, state, informFE=True):
        ...
        if deltaInfo[0]["state"] in ["activating", "activated"]:
            ...
            if not deltaInfo[0]["addition"]:
                self.logger.info(
                    "Failing delta %s. No addition parsed" % state["deltaid"]
                )
