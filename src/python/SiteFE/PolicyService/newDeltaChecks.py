    ...
    for connDelta in delta["addition"]:
        ...
        for state in ["committing", "committed", "activating", "activated"]:
            for deltat in ["addition", "modify"]:
                for dbdelta in dbObj.get(
                    "deltas", search=[["state", state], ["deltat", deltat]]
                ):
                    print(dbdelta)
...
def getdeltaInfo(deltaAd, delta):
    ...
    for hostname, hostdict in deltaAd["hosts"].items():
        ...
        try:
            ...
            raise DeltaKeyMissing(
                "New delta request does not have vlan information. Failing"
            )
    return out
...
def overlap_count(times1, times2):
    ...
    t1 = Range(
        start=datetime.fromtimestamp(times1[0]), end=datetime.fromtimestamp(times1[1])
    )
    t2 = Range(
        start=datetime.fromtimestamp(times2[0]), end=datetime.fromtimestamp(times2[1])
    )
...
def compareTwoDeltaTimes(deltaInDB, deltaIN):
    for dInhost, dInvlan in deltaIN["hosts"].items():
        if (
            dInhost in deltaInDB["hosts"].keys()
            and deltaInDB["hosts"][dInhost] == dInvlan
        ):
            overlap = overlap_count(deltaInDB["times"], deltaIN["times"])
            if overlap:
                msg = (
                    "New delta request overlaps with delta %s in %s state. Overlap time: %s"
                    % (deltaInDB["uid"], deltaInDB["state"], overlap)
                )
