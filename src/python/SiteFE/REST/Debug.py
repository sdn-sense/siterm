...
from SiteRMLibs.MainUtilities import dumpFileContentAsJson, getUTCnow
from SiteRMLibs.MainUtilities import getFileContentAsJson, generateRandomUUID
...
def getactionkeys(config, action):
    ...
    actionkeys = {
        "iperf-server": {
            ...
            "onetime": {"description": "One-time flag. If set, the server will only run for one test and then stop. If server finishes earlier than runtime parameter, iperf3 server will not be restarted.", "default": True, "required": False}
        },
        "iperf-client": {
        ...
        "fdt-server": {
            ...
            "onetime": {"description": "One-time flag. If set, the server will only run for one test and then stop. If server finishes earlier than runtime parameter, fdt process will not be restarted and task will finish.", "default": True, "required": False}
        },
        "fdt-client": {
            "time": {"description": "Duration of the test in seconds. Used as timeout <seconds> java -jar <jarfile> ... It must be lower than runtime duration", "default": None, "required": True},
        ...
        "rapid-ping": {"ip": {"description": "IP address to ping", "default": None, "required": True},
        ...
        "arp-table": {
            "interface": {"description": "Network interface to get the ARP table for", "default": None, "required": True}
        },
        "tcpdump": {
...
def getResponses(service):
    ...
    return {
        **{200:
            {"description": f"{service} debug information",
             "content": {
                "application/json": {
                    "example": {"TODO": "Add example response here"}
                }
            }},
                    404: {
                        "description": "Not Found. Possible Reasons:\n"
                                        " - No sites configured in the system."
                                        " - Service not configured in the system.",
...
def defaultkeys():
    ...
    return {
        ...
        "runtime": {"description": "Runtime duration in seconds. Instructs process to finish task after <seconds>. If not set, defaults between default and max runtime (random int).", "default": 600, "required": False},
...
# ==========================================================
@router.get("/{sitename}/debugactions",
            ...
            **DEFAULT_RESPONSES})
async def getDebugActionsList(sitename: str = Path(..., description="The site name to retrieve the debug actions list for."),
...
# =========================================================
@router.get("/{sitename}/debugactioninfo",
            ...
            **DEFAULT_RESPONSES})
async def getDebugActionInfo(sitename: str = Path(..., description="The site name to retrieve the debug action information for."),
...
@router.get("/{sitename}/debug",
            ...
            **DEFAULT_RESPONSES})
async def getDebugActions(sitename: str = Path(..., description="The site name to get the debug action information for."),
...
@router.get("/{sitename}/debug/{debugvar}",
            summary="Get Debug Action Information",
            description=("Returns the debug action information for the given debug ID."),
            tags=["Debug"],
            responses={
                200: {"description": "Debug action information",
                      "content": {
                          "application/json": {
                              "example": {"TODO": "Add example response here"}
                          }
                      }},
                404: {"description": "Debug request not found",
                      "content": {
                          "application/json": {
                              "example": {"detail": "Debug request with ID <debugvar> not found."}
                          }
                      }}
            })
async def getDebugInfo(sitename: str = Path(..., description="The site name to get the debug action information for."),
...
@router.put("/{sitename}/debug/{debugvar}",
                summary="Update Debug Action Information",
                description=("Updates the debug action information for the given debug ID."),
                tags=["Debug"],
                responses={
                    200: {"description": "Debug action information updated successfully",
                        "content": {
                            "application/json": {
                                "example": {"Status": "success", "ID": "<debug_id>"}
                            }
                        }},
                    404: {"description": "Debug request not found",
                        "content": {
...
async def updatedebug(item: DebugItem,
...
async def deletedebug(sitename: str = Path(..., description="The site name to get the debug action information for."),
...
@router.post("/{sitename}/debug",
             ...
             responses={
                 ...
                 400: {"description": "Bad request",
                       "content": {
                           "application/json": {
                               "example": {"detail": "Unsupported symbol in input request. Contact Support"}
                           }
                       }}
             })
async def submitdebug(item: DebugItem,
                      sitename: str = Path(..., description="The site name to get the debug action information for."),
                      deps=Depends(allAPIDeps)):
