...
from fastapi import APIRouter, Depends, HTTPException, status, Path, Request, Query, Response
from pydantic import BaseModel

from SiteRMLibs.MainUtilities import getModTime, decodebase64, getUTCnow, saveContent, httpdate
from SiteRMLibs.MainUtilities import getFileContentAsJson, removeFile, jsondumps, convertTSToDatetime
from SiteFE.REST.dependencies import allAPIDeps, depGetModel, checkSite
from SiteFE.REST.dependencies import DEFAULT_RESPONSES, checkReadyState
...
async def getServiceDeltas(request: Request,
    ...
    _rdfformat: Literal["turtle", "json-ld", "ntriples"] = Query("turtle", description="Model format: turtle, json-ld, ntriples."), # TODO: Implement this
...
@router.post("/{sitename}/deltas",
             ...
             **DEFAULT_RESPONSES})
async def submitDelta(item: DeltaItem,
    request: Request,
    ...
    if delta := _getdeltas(deps["dbI"], deltaID=item.id):
        ...
        if outContent["State"] not in ["accepted"]:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Failed to accept delta. Error Message: {outContent['Error']}. Full dump: {jsondumps(out)}"
            )
        return outContent
...
# =========================================================
@router.get("/{sitename}/deltas/{delta_id}",
            ...
            responses={**{
                ...
                404: {
                    ...
                    "content": {"application/json": {"example": {"detail": "Delta not found in the database."}}}
                },
                304: {
                    "description": "Delta not modified",
                    "content": {"application/json": {"example": {"detail": "Delta not modified."}}}
                }
            },
            **DEFAULT_RESPONSES})
...
@router.put("/{sitename}/deltas/{delta_id}/actions/{action}",
            ...
            responses={**{
                ...
                400: {
                    ...
                    "content": {"application/json": {"example": {"detail": "Invalid action or parameters."}}}
                },
                },
            **DEFAULT_RESPONSES})
...
# =========================================================
@router.get("/{sitename}/deltas/{delta_id}/timestates",
...
@router.post("/{sitename}/deltas/{delta_id}/timestates",
             ...
             responses={**{
                 ...
                 400: {
                     "description": "Bad Request",
                     "content": {"application/json": {"example": {"detail": "Invalid time state parameters."}}}
                 },
                 },
             **DEFAULT_RESPONSES})
async def createTimeStateForDelta(
    ...
    deps["dbI"].insert("deltatimestates", [{
    ...
    return Response(
        content={"status": "Time state created successfully", "delta_id": delta_id, "time_state": item.dict()},
