...
from SiteRMLibs.MainUtilities import HOSTSERVICES
...
@router.get("/{sitename}/services",
            ...
            **DEFAULT_RESPONSES})
async def getservice(_sitename: str = Path(..., description="The site name to retrieve the service for."),
...
@router.post("/{sitename}/services",
            ...
            **DEFAULT_RESPONSES})
async def createservice(item: ServiceItem,
                        sitename: str = Path(..., description="The site name to create the service for."),
                        deps=Depends(allAPIDeps)):
    """
...
@router.put("/{sitename}/services",
            ...
            **DEFAULT_RESPONSES})
async def updateservice(item: ServiceItem,
                        sitename: str = Path(..., description="The site name to update the service for."),
                        deps=Depends(allAPIDeps)):
    """
...
@router.delete("/{sitename}/services",
            ...
            responses={**{
                ...
                400: {
                    ...
                    "content": {
                        "application/json": {
                            "example": {"detail": "At least one of 'hostname' or 'servicename' query parameters must be provided."}
                        }
                    }},
                404: {
                    ...
                    "content": {
                        "application/json": {
                            "example": {"detail": "Service with hostname 'example' and servicename 'example' not found."}
                        }
                    }
                }},
            **DEFAULT_RESPONSES})
async def deleteservice(item: ServiceItem = Query(..., description="Service item to delete"),
    ...
    if not host:
        raise HTTPException(
            ...
            detail=f"Service with hostname '{item.hostname}' and servicename '{item.servicename}' not found."
        )
...
# GET
# ---------------------------------------------------------
@router.get("/{sitename}/servicestates",
...
@router.post("/{sitename}/servicestates",
             ...
             **DEFAULT_RESPONSES})
async def addservicestate(item: ServiceStateItem,
    sitename: str = Path(..., description="The site name to add/update the service state for."),
    deps=Depends(allAPIDeps)):
    """
    ...
    if not _host_supportedService(item.servicename):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"This Service {item.servicename} is not supported by Frontend"
        )
    try:
        dbOut = {"hostname": item.hostname,
        ...
        services = deps["dbI"].get("servicestates",
...
# GET
# ---------------------------------------------------------
@router.get("/{sitename}/serviceaction",
            ...
            responses={**{
                ...
                404: {
                    "description": "No service actions found for the given parameters.",
                    "content": {
                        "application/json": {
                            "example": {"detail": "No service actions found for the given parameters."}
                        }
                    }
                }
            },
...
async def getserviceaction(
    ...
    if not actions:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No service actions found for the given parameters."
        )
    return actions

# POST
# ---------------------------------------------------------
@router.post("/{sitename}/serviceaction",
             ...
             responses={**{
                 ...
                 400: {
                     ...
                     "content": {
                         "application/json": {
                             "example": {"detail": "No Services identified for params: {'servicename': 'ALL', 'action': 'start', 'hostname': 'ALL'}"}
                         }
                     }
                 }
             },
             **DEFAULT_RESPONSES})
async def serviceaction(item: ServiceActionItem,
    sitename: str = Path(..., description="The site name to record the service action for."),
    deps=Depends(allAPIDeps)):
    """
    ...
    for service in runningServices:
        ...
        if service["servicename"] in services:
            if item.hostname != "ALL" and service["hostname"] == item.hostname:
                ...
                statein = deps["dbI"].get("serviceaction",
                                  search=[["hostname", service["hostname"]],
...
# DELETE
# ---------------------------------------------------------
@router.delete("/{sitename}/serviceaction",
            ...
            responses={**{
                ...
                404: {
                    ...
                    "content": {
                        "application/json": {
                            "example": {"detail": "Service action not found for the given parameters."}
                        }
                    }
                }
            },
...
async def deleteserviceaction(
    ...
    _sitename: str = Path(..., description="The site name to delete the service action for."),
    deps=Depends(allAPIDeps)):
...
@router.post("/{sitename}/setinstancestartend",
...
async def setinstancestartend(
    ...
    sitename: str = Path(..., description="The site name to retrieve the instance start and end time for."),
    deps=Depends(allAPIDeps)):
    """
