...
async def getModelInfo(
    ...
    limit: int = Query(
        LIMIT_DEFAULT,
        description=f"The maximum number of results to return. Defaults to {LIMIT_DEFAULT}. Only applies if current is False.",
        ge=LIMIT_MIN,
        le=LIMIT_MAX,
    ),
...
async def getModelByID(
    ...
    summary: StrictBool = Query(
        False,
        description="Whether to return a summary of the model. Defaults to False.",
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Requested model not found in the database.",
        ) from ex
