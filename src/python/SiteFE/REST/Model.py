...
from fastapi import APIRouter, Depends, HTTPException, Response, status, Request, Query, Path
from fastapi.responses import JSONResponse

from SiteFE.REST.dependencies import allAPIDeps, checkSite
from SiteFE.REST.dependencies import DEFAULT_RESPONSES
from SiteFE.REST.dependencies import depGetModel, depGetModelContent

from SiteRMLibs.MainUtilities import getModTime, httpdate, convertTSToDatetime
...
@router.get("/{sitename}/models",
            ...
            responses={**{
                200: {
                    ...
                    "content": {"application/json": {"example": {"model": "example_model"}}}
                    },
                404: {
                    "description": "Model not found",
                    "content": {"application/json": {"example": {"detail": "Model not found in the database. First time run?"}}}
                },
                304: {
                    "description": "Not Modified",
                    "content": {"application/json": {"example": {"detail": "Model not modified since last request"}}}
                }
            },
            **DEFAULT_RESPONSES})
    ...
    try:
        if current:
            ...
            if not summary:
                ...
                return JSONResponse(content={"id": outmodels["uid"],
        ...
        for model in outmodels:
            tmpDict = {"id": model["uid"],
                       ...
                       "href": f"{request.base_url}api/{sitename}/models/{model['uid']}"}
            if not summary:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Model not found in the database. First time run?") from ex


# =========================================================
# /api/{sitename}/models/{modelID}
# =========================================================
@router.get("/{sitename}/models/{modelID}",
...
async def getModelByID(
    ...
    try:
        ...
        if not summary:
            ...
            return JSONResponse(content={"id": model["uid"],
                                     "creationTime": convertTSToDatetime(model["insertdate"]),
                                     "href": f"{request.base_url}api/{sitename}/models/{model['uid']}",
                                     "model": modContent},
                            media_type="application/json", headers=headers)
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Requested model not found in the database.") from ex
