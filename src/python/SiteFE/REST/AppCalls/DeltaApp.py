...
def deltas(environ, **kwargs):
    ...
    if not postRequest:
        if is_application_json(environ):
            ...
            kwargs["http_respond"].ret_400(
                "application/json", kwargs["start_response"], None
            )
            customErr = getCustomOutMsg(
                errMsg="You did POST method, but provided CONTENT_TYPE is not correct",
                errCode=400,
            )
    ...
    for key in ["modelId", "id"]:
        if not newDelta[key]:
            customErr = getCustomOutMsg(
                errMsg="You did POST method, %s is not specified" % key, errCode=400
            )
    ...
    if not newDelta["reduction"] and not newDelta["addition"]:
        customErr = getCustomOutMsg(
            errMsg="You did POST method, but nor reduction, nor addition is present",
            errCode=400,
        )
        print("Wrong delta: %s. Parsed:%s Error: %s" % (out, newDelta, customErr))
        kwargs["http_respond"].ret_400(
...
_DELTA_HOST_STATES_HISTORY_RE = re.compile(
    r"^/*v1/deltahoststateshistory/([-_A-Za-z0-9]+)/?$"
)
...
def deltas_id(environ, **kwargs):
    ...
    if environ["REQUEST_METHOD"].upper() == "DELETE":
        kwargs["http_respond"].ret_405(
            "application/json", kwargs["start_response"], ("Location", "/")
        )
    ...
    print(
        "Returning delta %s information. Few details: ModelID: %s, State: %s, LastModified: %s"
    ...
    if kwargs["urlParams"]["oldview"]:
        ...
        if not kwargs["urlParams"]["summary"]:
            current["model"] = encodebase64(
                DELTABACKEND.getmodel(outmodels[0]["uid"], content=True, **kwargs),
                kwargs["urlParams"]["encode"],
            )
        outM["models"].append(current)
        print(
            "Requested only current model. Return 200. Last Model %s"
            % outmodels[0]["uid"]
        )
        return [current]
...
def models_id(environ, **kwargs):
    ...
    if modTime > model["insertdate"]:
        print(
            "Model with ID %s was not updated so far. Time request comparison requested"
            % modelID
        )
        kwargs["http_respond"].ret_304(
            "application/json",
...
_DELTA_INTERNAL_ACTION_RE = re.compile(
    r"^/*v1/deltas/([-_A-Za-z0-9]+)/internalaction/([-_\.A-Za-z0-9]+)/(cancel|remove|active|activated|failed)/?$"
)
...
def delta_internal_actions(environ, **kwargs):
    ...
    msgOut = DELTABACKEND.commitdelta(
        deltaID, newState, internal=True, hostname=hostname, **kwargs
    )
    kwargs["http_respond"].ret_200("application/json", kwargs["start_response"], None)
    return msgOut


_DELTA_HOSTNAME_IDS_RE = re.compile(
    r"^/*v1/hostnameids/([-_\.A-Za-z0-9]+)/(activating|active)/?$"
)
...
CALLS = [
    (_DELTA_INTERNAL_ACTION_RE, delta_internal_actions, ["GET"], [], []),
    (
        _MODELS_ID_RE,
        models_id,
        ["GET"],
