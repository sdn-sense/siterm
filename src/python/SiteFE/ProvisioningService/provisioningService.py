...
class ProvisioningService(object):
    ...
    def pushInternalAction(self, url, state, deltaID, hostname):
        ...
        restOut = getDataFromSiteFE(
            {},
            url,
            "/sitefe/v1/deltas/%s/internalaction/%s/%s" % (deltaID, hostname, state),
        )
        if restOut[1] >= 400:
            msg = (
                "Failed to set new state in database for %s delta and %s hostname. Error %s "
    ...
    def deltaRemoval(
        self, newDelta, deltaID, newvlan, switchName, switchruler, fullURL
    ):
        """Here goes all communication with component and also rest
        interface."""
        self.logger.debug(
            "I got REDUCTION!!!!!. Reduction only sets states until active."
        )
        if "ReductionID" not in newDelta:
            # I dont know which one to set to removed...
            # Will apply only rules for reduction.
            self.pushInternalAction(
                fullURL, "remove", newDelta["ReductionID"], switchName
            )
        ...
        for stateChange in [
            ...
            if deltaState == list(stateChange.keys())[0]:
                msg = "Delta State %s and performing action to %s" % (
                    deltaState,
                    stateChange[deltaState],
                )
                self.logger.debug(msg)
                switchruler.mainCall(deltaState, newvlan, "remove")
                self.pushInternalAction(
                    fullURL, stateChange[deltaState], deltaID, switchName
                )
    ...
    def deltaCommit(self, newDelta, deltaID, newvlan, switchName, switchruler, fullURL):
        ...
        for stateChange in [
            ...
            if deltaState == list(stateChange.keys())[0]:
                self.logger.info(
                    "Delta State %s and performing action to %s"
                    % (deltaState, stateChange[deltaState])
                )
    ...
    def checkdeltas(self, switchHostname, inJson):
        ...
        if switchHostname in list(inJson["HostnameIDs"].keys()):
            for delta in inJson["HostnameIDs"][switchHostname]:
                ...
                self.logger.debug(
                    "Switch %s delta state %s" % (delta, inJson["ID"][delta]["State"])
                )
                if inJson["ID"][delta]["State"] in ["activating", "removing"]:
                    if inJson["HostnameIDs"][switchHostname][delta] not in [
                        "failed",
                        "active",
                        "remove",
                    ]:
                        tmpDict = inJson["ID"][delta]
                        tmpDict["HOSTSTATE"] = inJson["HostnameIDs"][switchHostname][
                            delta
                        ]
        ...
        for switchName in list(list(switches["switches"].keys()) + alliases):
            ...
            for newDelta in newDeltas:
                ...
                for actionKey in ["reduction", "addition"]:
                    try:
                        newvlan = self.getnewvlan(
                            newDelta, newDelta["ID"], switchName, actionKey
                        )
                        if (
                            actionKey == "reduction"
                            and newDelta["ParsedDelta"][actionKey]
                        ):
                            self.deltaRemoval(
                                newDelta,
                                newDelta["ID"],
