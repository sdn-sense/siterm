...
class LookUpService:
    ...
    def generateHostIsalias(self, **kwargs):
        ...
        if kwargs["portSwitch"] in list(self.hosts.keys()):
            for item in self.hosts[kwargs["portSwitch"]]:
                if (
                    item["switchName"] == kwargs["switchName"]
                    and item["switchPort"] == kwargs["portName"]
                ):
                    suri = "%s:%s" % (kwargs["newuri"], item["intfKey"])
                    self.newGraph.add(
                        (
                            self.prefixDB.genUriRef("site", kwargs["newuri"]),
    ...
    def addToGraph(self, sub, pred, obj):
        ...
        if len(obj) == 1 and len(sub) == 1:
            ...
            self.newGraph.add(
                (
                    self.prefixDB.genUriRef(sub[0], sub[1]),
                    self.prefixDB.genUriRef(pred[0], pred[1]),
                    self.prefixDB.genLiteral(obj[0]),
                )
            )
        ...
        for dKey, dMappings in list(mappings.items()):
            for mapping in dMappings:
                ...
                if mapping in list(inputDict[dKey].keys()) and inputDict[dKey][mapping]:
                    ...
                    self.addToGraph(
                        ["site", "%s:%s" % (prefixuri, mName)], ["mrs", "type"], [mName]
                    )
                    self.addToGraph(
                        ["site", "%s:%s" % (prefixuri, mName)],
                        ["mrs", "value"],
    ...
    def _deltaReduction(self, dbObj, delta, mainGraphName):
        ...
        self.logger.info(
            "Main Graph len: %s Reduction Len: %s", len(mainGraph), len(tmpGraph)
        )
        mainGraph -= tmpGraph
        dbObj.update(
            "deltasmod",
            [{"uid": delta["uid"], "updatedate": getUTCnow(), "modadd": "removed"}],
        )
        return mainGraph
    ...
    def _addDeltaStatesInModel(self, mainGraph, state, addition):
        ...
        for conn in addition:
            if "timestart" in list(conn.keys()) and state == "committed":
                ...
                "timeend" in list(conn.keys())
                and conn["timeend"] < getUTCnow()
                and state == "activated"
            ):
                state = "deactivating"
            ...
            if "timestart" in list(conn.keys()) or "timeend" in list(conn.keys()):
                mainGraph.add(
                    (
                        self.prefixDB.genUriRef(
                            custom="%s:lifetime" % conn["connectionID"]
                        ),
                        self.prefixDB.genUriRef("mrs", "tag"),
                        self.prefixDB.genLiteral("monitor:status:%s" % state),
                    )
    ...
    def _deltaAddition(self, dbObj, delta, mainGraphName, updateState=True):
        ...
        os.unlink(tmpFile.name)
        self.logger.info(
            "Main Graph len: %s Addition Len: %s", len(mainGraph), len(tmpGraph)
        )
        mainGraph += tmpGraph
        # Add delta states for that specific delta
        mainGraph = self._addDeltaStatesInModel(
            mainGraph, delta["state"], evaldict(delta["addition"])
        )
        if updateState:
            dbObj.update(
                "deltasmod",
                [{"uid": delta["uid"], "updatedate": getUTCnow(), "modadd": "added"}],
            )
        return mainGraph
        ...
        for modstate in ["added", "add", "remove"]:
            for delta in dbObj.get("deltas", search=[["modadd", modstate]], limit=10):
                ...
                if delta["deltat"] == "reduction":
                    ...
                    delta["modadd"] in ["add"]
                    or delta["state"] in ["activated", "activating", "committed"]
                ):
    ...
    def defineMRMLServices(self):
        ...
        for switchName in self.config.get(self.sitename, "switch").split(","):
            ...
            try:
                ...
                self.logger.debug(
                    "Labelswapping parameter is not defined. By default it is set to False."
                )
            self.newGraph.add(
                (
                    self.prefixDB.genUriRef("site", ":service+vsw:%s" % vsw),
        ...
        try:
            self.newGraph.add(
                (
                    ...
                    self.prefixDB.genLiteral(
                        self.config.get(self.sitename, "latitude")
                    ),
                )
            )
            self.newGraph.add(
                (
                    ...
                    self.prefixDB.genLiteral(
                        self.config.get(self.sitename, "longitude")
                    ),
                )
            )
        except configparser.NoOptionError:
    ...
    def defineLayer3MRML(self, nodeDict):
        ...
        self.newGraph.add(
            (
                ...
                self.prefixDB.genUriRef("nml", "hasService"),
                self.prefixDB.genUriRef(
                    "site", ":%s:service+rst" % nodeDict["hostname"]
                ),
            )
        )
        self.newGraph.add(
        ...
        self.newGraph.add(
            (
                ...
                self.prefixDB.genLiteral(
                    "http://services.ogf.org/nsi/2019/08/descriptions/l3-vpn"
                ),
            )
        )
        ...
        for tablegress in ["table+defaultIngress", "table+defaultEgress"]:
            ...
            for routeinfo in hostinfo["NetInfo"]["routes"]:
                routename = ""
                if (
                    "RTA_DST" in list(routeinfo.keys())
                    and routeinfo["RTA_DST"] == "169.254.0.0"
                ):
                ...
                if "RTA_GATEWAY" in list(routeinfo.keys()):
                    ...
                    if "RTA_PREFSRC" not in list(routeinfo.keys()):
                        continue
                    routename = routingtable + ":route+%s_%s" % (
                        routeinfo["RTA_PREFSRC"],
                        routeinfo["dst_len"],
                    )
                self.newGraph.add(
                    (
                ...
                if "RTA_GATEWAY" in list(routeinfo.keys()):
                    ...
                    self.newGraph.add(
                        (
                            ...
                            self.prefixDB.genUriRef(
                                "site", "%s:%s" % (routename, "black-hole")
                            ),
                        )
                    )
                    for vals in [
                        ["to", "ipv4-prefix-list", "0.0.0.0/0"],
                        ["black-hole", "routing-policy", "drop"],
                        ["local", "routing-policy", "local"],
                    ]:
                        self.addToGraph(
                            ["site", "%s:%s" % (routename, vals[0])],
                            ["rdf", "type"],
                            ["mrs", "NetworkAddress"],
                        )
                        self.addToGraph(
            ...
            for portName, portSwitch in list(switchDict.items()):
                ...
                if switchName in list(switchInfo["vlans"].keys()):
                    if portName in list(switchInfo["vlans"][switchName].keys()):
                        ...
                        if (
                            "vlan_range"
                            in list(switchInfo["vlans"][switchName][portName].keys())
                            and switchInfo["vlans"][switchName][portName]["vlan_range"]
                        ):
                            self.newGraph.add(
                                (
                                    self.prefixDB.genUriRef("site", newuri),
                            ...
                            self.newGraph.add(
                                (
                                    ...
                                    self.prefixDB.genLiteral(
                                        switchInfo["vlans"][switchName][portName][
                                            "vlan_range"
                                        ]
                                    ),
                                )
                        ...
                        if (
                            "capacity"
                            in list(switchInfo["vlans"][switchName][portName].keys())
                            and switchInfo["vlans"][switchName][portName]["capacity"]
                        ):
                            self.newGraph.add(
                                (
                                    self.prefixDB.genUriRef("site", newuri),
                        ...
                        if (
                            "isAlias"
                            in list(switchInfo["vlans"][switchName][portName].keys())
                            and switchInfo["vlans"][switchName][portName]["isAlias"]
                        ):
                            self.newGraph.add(
                                (
                                    self.prefixDB.genUriRef("site", newuri),
                                    ...
                                    self.prefixDB.genUriRef(
                                        "",
                                        custom=switchInfo["vlans"][switchName][
                                            portName
                                        ]["isAlias"],
                                    ),
                                )
                    ...
                    self.logger.debug(
                        "switchName $%s is not in switchInfo" % switchName
                    )
        ...
        if "vlan_range" in list(intfDict.keys()):
            self.newGraph.add(
                (
                    ...
                    self.prefixDB.genUriRef("nml", "hasService"),
                    self.prefixDB.genUriRef(
                        "site", "%s:%s" % (newuri, "bandwidthService")
                    ),
                )
            )
            self.newGraph.add(
                (
                    ...
                    self.prefixDB.genUriRef("nml", "isAlias"),
                    self.prefixDB.genUriRef(
                        "site", ":%s:%s:+" % (switchName, switchPort)
                    ),
                )
            )
            ...
            self.newGraph.add(
                (
                    self.prefixDB.genUriRef("site", bws),
                    self.prefixDB.genUriRef("rdf", "type"),
    ...
    def defineHostInfo(self, nodeDict):
        ...
        for intfKey, intfDict in list(hostinfo["NetInfo"]["interfaces"].items()):
            ...
            self.hosts[nodeDict["hostname"]].append(
                {"switchName": switchName, "switchPort": switchPort, "intfKey": intfKey}
            )
            newuri = ":%s:%s:%s:%s" % (
                switchName,
                switchPort,
        ...
        for model in dbObj.get("models", limit=100, orderby=["insertdate", "ASC"]):
            if model["insertdate"] < int(getUTCnow() - 86400):
                ...
                try:
                    ...
                    self.logger.debug(
                        "Got OS Error removing this model %s. Exc: %s"
                        % (model, str(ex))
                    )
