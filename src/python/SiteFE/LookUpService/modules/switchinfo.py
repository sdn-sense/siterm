...
class SwitchInfo:
    ...
    def addSwitchIntfInfo(self, switchName, portName, portSwitch, newuri):
        ...
        for key, val in portSwitch.items():
            ...
            if key == "vlan_range_list":
                ...
                self._addIsAlias(
                    uri=newuri,
                    isAlias=portSwitch.get("isAlias"),
                    hostname=switchName,
                    portName=portName,
                    nodetype="switch",
                )
                ...
                for value in val:
                    ...
                    self.addToGraph(
                        ...
                        [
                            "site",
                            f"{switchuri}:{self.switch.getSystemValidPortName(value)}",
                        ],
                    )
    ...
    def _addSwitchVlanLabel(self, vlanuri, value):
        ...
        self.addToGraph(["site", vlanuri], ["nml", "hasLabel"], ["site", labeluri])
        self.addToGraph(["site", labeluri], ["rdf", "type"], ["nml", "Label"])
        self.addToGraph(["site", labeluri], ["nml", "labeltype"], ["schema", "#vlan"])
        self.setToGraph(["site", labeluri], ["nml", "value"], [str(value)])
    ...
    def _addSwitchVlanInfo(self, key, switchInfo):
        ...
        for switchName, switchDict in list(switchInfo[key].items()):
            ...
            try:
                ...
                self.logger.debug(
                    "ERROR: vsw parameter is not defined for %s. Err: %s",
                    switchName,
                    ex,
                )
                continue
