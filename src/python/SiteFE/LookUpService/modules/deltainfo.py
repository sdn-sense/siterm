...
class DeltaInfo:
    ...
    def _addParams(self, params, uri, sublevel=False):
        ...
        for key in [
            ["tag", "mrs", True, None],
            ["labelSwapping", "nml", True, XSD.boolean],
            ["belongsTo", "nml", False, None],
            ["encoding", "nml", False, None],
        ]:
            val = ""
        ...
        for ipkey in ["ipv4", "ipv6"]:
            ...
            if val and uri:
                ...
                self._addVals(
                    ...
                    val,
                    uri,
                    **{"labeluri": ipdict.get("uri", ""), "delta": True},
                )
                ...
                for iptype in iptypes.keys():
                    ...
                    for route, routeInfo in routedict.get("hasRoute", {}).items():
                        ...
                        self._addBandwidthServiceRoute(
                            **{
                                "routeuri": route,
                                "uri": routedict.get("hasService", {}).get("uri", ""),
                            }
                        )
