...
def generateVal(cls, inval, inkey, esc=False):
    ...
    if isinstance(inval, (dict, list)):
        cls.logger.info(
            f"Out is dictionary/list, but vals unknown. Return as str {inval}"
        )
        return str(inval)
...
def _genIPv4(cls, inval, inkey, esc=True):
    ...
    if isinstance(inval, list):
        tmpKeys = [
            {"key": inkey, "subkey": _genIPv4(cls, val, inkey), "val": val}
            for val in inval
        ]
        return tmpKeys
    cls.logger.debug(
        f"No IPv4 value. Return empty String. This might break things. Input: {inval} {inkey}"
    )
    return ""
...
def _genIPv6(cls, inval, inkey, esc=True):
    ...
    if isinstance(inval, list):
        ...
        return tmpKeys
    cls.logger.debug(
        f"No IPv6 value. Return empty String. This might break things. Input: {inval} {inkey}"
    )
    return ""
...
class RDFHelper:
    ...
    def generateKey(self, inval, inkey):
        ...
        self.logger.debug(
            f"Generate Keys return empty. Unexpected. Input: {inval} {inkey}"
        )
        return ""
    ...
    def getSavedPrefixes(self, additionalhosts=None):
        ...
        prefixSite = (
            f"{self.config.get('prefixes', 'site')}"
        ...
        for switchName in self.config.get(self.sitename, "switch"):
            for key in ["vsw", "rst"]:
                try:
                    ...
                    if tKey != switchName:
                        self.logger.debug(
                            f"Config mistake. Hostname != {key} ({switchName} != {tKey})"
                        )
                        continue
                    prefixes.setdefault(key, {})
                    prefixes[key][
                        switchName
                    ] = f"{prefixes['site']}:{tKey}:service+{key}"
    ...
    def _addVals(self, key, subkey, val, newuri, **kwargs):
        ...
        if key in ["ipv4", "ipv6"]:
            ...
            if isinstance(val, dict):
                ...
                self.logger.info(
                    f"Unexpected value for IP. Skipping entry. Input data: {key}, {subkey}, {val}, {newuri}"
                )
            if not tmpVal:
                return
            if kwargs.get("delta", False):
                labeluri = (
                    self.URIs.get("ips", {})
                    .get(tmpVal, {})
                    .get("uri", f"{newuri}:{key}-address+{validMRMLName(tmpVal)}")
                )
                reptype = (
                    self.URIs.get("ips", {})
                    .get(tmpVal, {})
                    .get("type", f"{key}-address|unverifiable")
                )
            else:
        ...
        self.addToGraph(
            ["site", newuri], ["mrs", "hasNetworkAddress"], ["site", labeluri]
        )
    ...
    def _addMetadataService(self, uri=""):
        ...
        triples = [(self.genUriRef("site", uri),
    ...
    def _addMetadata(self, **kwargs):
        ...
        if "webdomain" in kwargs and kwargs["webdomain"]:
            ...
            if "sitename" in kwargs and kwargs["sitename"]:
                self._addHasNetworkAttribute(
                    metaService, "sitename", "/sitename", kwargs["sitename"]
                )
        ...
        try:
            ...
            for key, vals in metadata.items():
                self._addHasNetworkAttribute(
                    metaService, key, f"/{key}", json.dumps(vals)
                )
    ...
    def _addPort(self, **kwargs):
        ...
        triples = [(self.genUriRef("site", f":{kwargs['hostname']}"),
    ...
    def _addSwitchingService(self, **kwargs):
        ...
        if kwargs["vsw"] != kwargs["hostname"]:
            self.logger.debug(
                f"Config mistake. Hostname != vsw ({kwargs['hostname']} != {kwargs['vsw']})"
            )
            return ""
    ...
    def _addServiceDefinition(self, **kwargs):
        ...
        triples = [(self.genUriRef("site", kwargs["uri"]),
    ...
    def _addSwitchingSubnet(self, **kwargs):
        ...
        triples = [(self.genUriRef("site", svcService),
    ...
    def _addPortSwitchingSubnet(self, **kwargs):
        ...
        self.addTriples([(self.genUriRef("site", kwargs["subnet"]),
                          self.genUriRef("nml", "hasBidirectionalPort"),
    ...
    def _addBandwidthServiceRoute(self, **kwargs):
        if self.__checkifReqKeysMissing(["routeuri", "uri"], kwargs):
            return
        self.addTriples([(self.genUriRef("site", kwargs["routeuri"]),
                          self.genUriRef("nml", "hasService"),
                          self.genUriRef("site", kwargs["uri"]))])
    ...
    def _addRoutingService(self, **kwargs):
        ...
        self.addTriples([(self.genUriRef("site", uri),
                          self.genUriRef("nml", "hasService"),
    ...
    def _addRoute(self, **kwargs):
        ...
        if "routeuri" in kwargs and kwargs["routeuri"]:
            ...
            return ""
        self.addTriples([(self.genUriRef("site", ruri),
    ...
    def _addProvidesRoute(self, **kwargs):
        ...
        if not suri or not kwargs["routeuri"]:
            return ""
        self.addTriples([(self.genUriRef("site", suri),
    ...
    def _addNetworkAddressEntry(self, **kwargs):
        ...
        self.addToGraph(
            ["site", kwargs["uri"]], ["rdf", "type"], ["mrs", "NetworkAddress"]
        )
    ...
    def addVlanRange(self, **kwargs):
        ...
        # kwargs schema - #vlan
        # kwargs values, list
        self.addTriples([(self.genUriRef("site", kwargs["newuri"]),
        ...
        if kwargs.get("schema"):
            self.addTriples([(self.genUriRef("site", f"{kwargs['newuri']}:{kwargs['name']}"),
    ...
    def _addVswPort(self, **kwargs):
        ...
        if not uri or not vsw:
            return ""
        self.addTriples([(self.genUriRef("site", vsw),
                          self.genUriRef("nml", "hasBidirectionalPort"),
    ...
    def _addRstPort(self, **kwargs):
        ...
        if not uri:
            return ""
        iptypes = []
        if kwargs.get("nodetype", "") == "switch" and kwargs.get(
            "hostname", "no-host"
        ) in self.prefixes.get("rst", {}):
            try:
    ...
    def _addSwitchingVlanLabel(self, **kwargs):
        ...
        try:
            ...
            globalvlan = self.config.get(
                kwargs["hostname"], "globalvlan", f"{vsw}_globalvlan"
            )
        except (NoSectionError, NoOptionError):
            return
        if vsw and globalvlan:
            svcService = f":{vsw}:service+vsw"
            self.addTriples([(self.genUriRef("site", svcService),
    ...
    def _addVlanPort(self, **kwargs):
        ...
        self.addTriples([(self.genUriRef("site", vlanuri),
                          self.genUriRef("rdf", "type"),
                          self.genUriRef("nml", "BidirectionalPort"))])
        return vlanuri
    ...
    def _addHasNetworkAttribute(self, uri, name, vtype, value):
        ...
        self.addToGraph(
            ["site", uri], ["mrs", "hasNetworkAttribute"], ["site", f"{uri}:{name}"]
        )
        self.addToGraph(
            ["site", f"{uri}:{name}"], ["rdf", "type"], ["mrs", "NetworkAttribute"]
        )
        self.addToGraph(
            ["site", f"{uri}:{name}"], ["mrs", "type"], ["metadata:directory"]
        )
