...
class ConfigFetcher:
    ...
    def _fetchFile(self, name, url):
        def retryPolicy(outObj, retries=3):
            if "status_code" in outObj and outObj["status_code"] == -1:
                self.logger.debug(
                    f"Got -1 (Timeout usually error. Will retry up to 3 times (5sec sleep): {outObj}"
                )
                retries -= 1
            ...
            if retries == 0:
                self.logger.debug(
                    f"Got too many retries. Will stop retrying to get config file: {outObj}"
                )
            else:
                time.sleep(5)
            return retries
        ...
        output = {}
        datetimeNow = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(
            minutes=10
        )
        ...
        if os.path.isfile(filename):
            ...
            prevfilename = (
                f"/tmp/{datetimelasthour.strftime('%Y-%m-%d-%H')}-{name}.yaml"
            )
            ...
            while retries > 0:
                ...
                try:
                    ...
                    if os.path.isfile(prevfilename):
                        self.logger.info(
                            f"Remove previous old cache file {prevfilename}"
                        )
    ...
    def fetchAgent(self):
        ...
        if self.gitObj.config["MAPPING"]["type"] == "Agent":
            url = self.gitObj.getFullGitUrl(
                [self.gitObj.config["MAPPING"]["config"], "main.yaml"]
            )
    ...
    def fetchFE(self):
        ...
        if self.gitObj.config["MAPPING"]["type"] == "FE":
            ...
            url = self.gitObj.getFullGitUrl(
                [self.gitObj.config["MAPPING"]["config"], "auth.yaml"]
            )
            self._fetchFile("FE-auth", url)
            url = self.gitObj.getFullGitUrl(
                [self.gitObj.config["MAPPING"]["config"], "auth-re.yaml"]
            )
    ...
    def startwork(self):
        ...
        self.gitObj.config["MAPPING"] = copy.deepcopy(
            mapping[self.gitObj.config["MD5"]]
        )
        self.fetchAgent()
        self.fetchFE()
