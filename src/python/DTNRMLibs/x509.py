...
class CertHandler(object):
    ...
    def validateCertificate(self, environ):
        ...
        if environ["CERTINFO"]["notBefore"] > timestamp:
            print(
                "Certificate Invalid. Current Time: %s NotBefore: %s"
                % (timestamp, environ["CERTINFO"]["notBefore"])
            )
            raise Exception("Certificate Invalid")
        # Check time after
        if environ["CERTINFO"]["notAfter"] < timestamp:
            print(
                "Certificate Invalid. Current Time: %s NotAfter: %s"
                % (timestamp, environ["CERTINFO"]["notAfter"])
            )
            raise Exception("Certificate Invalid")
        # Check DN in authorized list
        if environ["CERTINFO"]["fullDN"] not in list(self.allowedCerts.keys()):
            print(
                "User DN %s is not in authorized list" % environ["CERTINFO"]["fullDN"]
            )
            raise Exception("Unauthorized access")
