...
def getdeltaAll(sitename):
    ...
    for delta in dbobj.get("deltas"):
        ...
        if delta["deltat"] not in ["reduction", "addition"]:
            print(
                "SOMETHING WRONG WITH THIS DELTA. It does not have any type defined. Was not parsed properly"
            )
            continue
        ...
        for conn in conns:
            if "hosts" not in list(conn.keys()):
                print(
                    "SOMETHING WRONG WITH THIS DELTA. It does not have any hosts defined."
                )
                continue
            for hostname in list(conn["hosts"].keys()):
                print("-" * 20)
                print("Host States %s" % hostname)
                for hoststate in dbobj.get(
                    "hoststates",
        ...
        for conn in conns:
            for dConn in dbobj.get(
                "delta_connections", search=[["connectionid", conn["connectionID"]]]
            ):
                print(dConn)
